...
const Event = () => {
  ...
  const fetchEvents = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:9080/api/events?${queryParams}`);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {events.length > 0 ? (
              ...
              <div key={index} className="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300">
                <div className="relative">
                  {event.eventType === 'image' && (
                    <img src={event.eventFile} alt={event.eventName} className="w-full h-48 object-cover" />
                  )}
                ...
                <div className="p-4 sm:p-6">
                  <h2 className="text-xl sm:text-2xl font-bold mb-2 text-gray-800">{event.eventName}</h2>
                  ...
                  {event.eventDescription && (
                    <p className="text-gray-600 text-sm sm:text-base line-clamp-2">{event.eventDescription}</p>
                  )}
            ...
            <div className="col-span-3 flex justify-center items-center h-64">
              <p className="text-center text-gray-600 text-lg">No events found. Try adjusting your search or filters.</p>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="flex space-x-1">
            {Array.from({ length: totalPages }, (_, index) => (
              <button
                ...
                className={`px-3 py-2 ${
                  page === index + 1 ? 'bg-blue-500 text-white' : 'bg-white text-black border border-gray-300'
